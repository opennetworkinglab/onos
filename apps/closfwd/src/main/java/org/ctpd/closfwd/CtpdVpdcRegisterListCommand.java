 ...
 */
@Service
@Command(scope = "onos", name = "clos-vpdc-registry-list",
        description = "Access the content of CTPD registry")
public class CtpdVpdcRegisterListCommand extends AbstractShellCommand {
    ...
    protected void doExecute() {
        ...
        if(uuid==null)
        {
            Iterator<UUID> uuids = service.getVpdcRegisterUUIDs().iterator();
            while(uuids.hasNext())
            {
        ...
        {
            Endpoint device = service.getEndpoint(UUID.fromString(uuid));
            if(device!=null)
            {
