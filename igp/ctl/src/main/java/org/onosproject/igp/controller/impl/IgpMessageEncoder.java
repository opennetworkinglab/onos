 ...
 * netty pipeline.
 */
public class IgpMessageEncoder extends OneToOneEncoder {
    ...
    protected Object encode(ChannelHandlerContext ctx, Channel channel,
        ...
        List<FlowRuleBatchExtRequest> msglist = (List<FlowRuleBatchExtRequest>) msg;
        ...
        for (FlowRuleBatchExtRequest ofm : msglist) {
            int batchId = ofm.batchId();
            buf.writeInt(batchId);

            Collection<FlowRuleExtEntry> toAdd = ofm.getBatch();
            for(FlowRuleExtEntry flowrule : toAdd) {
            	buf.writeBytes(flowrule.getFlowEntryExt());
            }	
        }
        return buf;
    }
