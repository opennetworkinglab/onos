...
public final class EncodeCriterionCodecHelper {
    ...
    private class FormatExtension implements CriterionTypeFormatter {
        @Override
        public ObjectNode encodeCriterion(ObjectNode root, Criterion criterion) {
            // use the ExtensionCriterion serializer inside the API because
            // specific implementations (codecs) for ExtensionSelector are not allowed here
            Kryo kryo = KryoNamespaces.API.borrow();
            Output output = new Output(new ByteArrayOutputStream());
            kryo.writeObject(output, criterion);
